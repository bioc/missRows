...
test_that("MIDTList object from a list of data tables", {
    ...
    midt <- newMIDTList(tableList, colData=colData, 
...
test_that("tests for data tables", {
    ...
    expect_error(newMIDTList(table1, colData=colData), 
            "at least two data tables must be passed as arguments in '...'")
    
    tableNrn <- table1
    colnames(tableNrn) <- NULL
    expect_error(newMIDTList(tableNrn, table2, colData=colData), 
                "the 'Table 1' data table must be columns named.")
    
    expect_error(newMIDTList(1:nrow(table1), table2, colData=colData), 
                "the 'Table 1' data table must be a matrix or data frame.")
    
    tableInf <- table1
    tableInf[1, 1] <- Inf
    expect_error(newMIDTList(tableInf, table2, colData=colData), 
