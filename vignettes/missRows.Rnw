...
%============================================================================
\Rpackage{missRows} is an \R{} package that implements the MI-MFA approach
\cite{Voillet_2016} ...
...
\Rcode{NCI-60} data. We first load the data \medskip
...
\Rcode{missPattern} calculate and plot the amount of missing/available rows 
in each stratum per data table. Data tables are plotted separately on a same
device showing the pattern of missingness. The individuals are colored with
rapport to their stratum whereas missing rows are colored according to the
\Rcode{col.missing} argument.
...
imputed individuals are colored according to the \Rcode{col.missing} 
argument. \medskip
...
the missing row values. In the \Rpackage{missRows} package we propose two
...
setting the \Rcode{conf.areas} argument to \Rcode{'ellipse'} \medskip
...
Convex hulls are plotted by setting the \Rcode{conf.areas} argument to
...
with concentric circles of radius one and radius given by \Rcode{rad.in}.
...
\Rcode{M = seq(inc, M.max, by = inc)}. The stability of the estimated MI-MFA
...
\caption{\textbf{Overview of our MI-MFA approach to handling missing rows in
...
second step, multiple imputation is performed using the hot-deck imputation
...
entries, but differ in the imputed values. The third step is to estimate the
...
\boldsymbol{F}_{\!M}$ into a compromise configuration 
$\boldsymbol{F_{\!c}}$ using the STATIS method.}
