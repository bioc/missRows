newMIDTList <- function(..., strata = NULL, tableNames = NULL) {
  ...
  for (i in 1:length(K)) {
    if (any(apply(K[[i]], 1, is.infinite))) {
      stop("infinite values in '", names(K)[i], "' data table.", call. = FALSE)
    }
  }
  ...
  if (length(unique(nr)) != 1)
    stop("non equal row numbers among tables and/or strata", call. = FALSE)
  ...
  for (i in 1:length(K)) {
    if (!identical(rnames, rownames(K[[i]]))) {
      stop("non equal row names among tables and/or strata", call. = FALSE)
    }
  }
  ...
  if (length(unlist(miss)) == 0)
    stop("no missing rows in the data tables, MI is not useful. Perform MFA.",
         call. = FALSE)
